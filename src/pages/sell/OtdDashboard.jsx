...
const OtdDashboard = () => {
    ...
    try {
      ...
      plansRaw.forEach(plan => {
          ...
          if (plan.status === 'Canceled') {
              ...
              // Only status='Completed' AND informed_after_dispatch counts as delivered
      ...
      const mappedOrders = ordersRaw.map(o => {
          ...
          const remainingQty = parseFloat(o.qty) || 0;  // app_orders.qty = remaining after cancellations
          const cancelledQty = stats.cancel;
          const totalOriginalQty = remainingQty + cancelledQty;  // original = remaining + all cancelled
          return {
              ...o,
              orderDate: o.order_date,
              clientName: o.client_name,
              qty: remainingQty,
              totalOriginalQty,          // original customer order qty
              cancelQty: cancelledQty,
              planningQty: stats.planned,
              qtyDelivered: stats.delivered,
              planningPendingQty: remainingQty - stats.planned, // Allowed to be negative for over-planning
              columnQ: "Available",
  ...
  useEffect(() => {
    ...
    // 1. Core Sums
    const remainingQtySum  = orders.reduce((sum, item) => sum + item.qty, 0);   // app_orders.qty = remaining
    const cancelQtySum     = orders.reduce((sum, item) => sum + (item.cancelQty || 0), 0);
    const totalOriginalQtySum = remainingQtySum + cancelQtySum;                 // original = remaining + cancelled
    // Delivered: only status='Completed' AND informed_after_dispatch = true
    const deliveredQtySum  = planningData.reduce((sum, item) => sum + (item.status === 'Completed' && item.informed_after_dispatch ? item.dispatchQty : 0), 0);
    const cancelQtyPlanSum = planningData.reduce((sum, item) => sum + (item.status === 'Canceled' ? item.dispatchQty : 0), 0);
    // In Process: active plans that are not Canceled, AND not fully Delivered
    const inProcessQtySum  = planningData.reduce((sum, item) => sum + (item.status !== 'Canceled' && !(item.status === 'Completed' && item.informed_after_dispatch) ? item.dispatchQty : 0), 0);
    // To Plan: positive remaining planning quantity across all orders
    const toPlanQtySum     = orders.reduce((sum, item) => sum + (item.planningPendingQty > 0 ? item.planningPendingQty : 0), 0);
    ...
    // 5. Stage 4: Inform After Dispatch
    // ONLY applies to completed dispatches — Stage 4 total must always = Stage 3 done
    // This prevents Stage4Done > Stage3Done impossibility
    const completedPlans = planningData.filter(p => p.status === 'Completed');
    const stage4Done    = completedPlans.filter(p =>  p.informed_after_dispatch).length;
    const stage4Pending = completedPlans.filter(p => !p.informed_after_dispatch).length;
    ...
    setStats({
      orderQtySum: remainingQtySum,
      cancelQtySum: cancelQtyPlanSum,
      remainingQtySum: orders.reduce((sum, item) => sum + item.planningPendingQty, 0),
      deliveredQtySum,
      totalOriginalQtySum,
      toPlanQtySum,
      overPlannedQtySum,
    ...
    <div className="min-h-screen bg-[#F5F5F5]">
      <div className="p-3 sm:p-6 space-y-6">
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            <StatCard
              title="Total (Original) Qty"
              value={(stats.totalOriginalQtySum || 0).toLocaleString()}
              icon={FileText}
              color="text-primary"
              bgColor="bg-green-50"
            />
            <StatCard
              title="Remaining (Order) Qty"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            <StatCard
              title="To Plan Qty"
              value={stats.toPlanQtySum.toLocaleString()}
              icon={ClipboardCheck}
              color="text-orange-500"
              bgColor="bg-orange-50"
            />
            <StatCard
              title="Over Planned Qty"
              value={stats.overPlannedQtySum.toLocaleString()}
              icon={Activity}
              color="text-red-600"
              bgColor="bg-red-50"
            />
            <StatCard
              ...
              value={stats.inProcessQtySum.toLocaleString()}
              icon={Clock}
