...
const Products = ({ isTab = false }) => {
    ...
    const parseItemName = (raw) => {
        ...
        while ((match = regex.exec(name)) !== null) {
            segments.push({ full: match[0], inner: match[1].trim() });
        }

        segments.reverse().forEach(seg => {
            if (/^\d+\s*[\*xX]\s*\d+$/.test(seg.inner)) {
                product_type = seg.inner.replace(/\s+/g, '').toUpperCase().replace(/\*/g, 'X');
                name = name.replace(seg.full, '');
            } else if (/^\d+\s*KG$/i.test(seg.inner)) {
                mux = seg.inner.toUpperCase();
                name = name.replace(seg.full, '');
            }
        });
