...
const Settings = () => {
    ...
    const validateForm = (data) => {
        ...
        // User ID is only required if we are editing an existing user (to identify them)
    ...
    const handleSubmit = async (e) => {
        ...
        const cleanedData = {
            ...formData,
            user_id: editingUser ? formData.user_id?.trim() : undefined,
        ...
        if (editingUser) {
            const localDuplicate = users.find(u =>
                u.user_id === cleanedData.user_id && u.user_id !== editingUser.user_id
            );
            if (localDuplicate) {
                setErrors(prev => ({ ...prev, user_id: 'Duplicate User ID found' }));
        ...
        try {
            ...
            if (cleanedData.user_id) {
                checks.push(supabase.from('users').select('user_id').eq('user_id', cleanedData.user_id));
            }

            const [usernameCheck, userCheck] = await Promise.all(checks);

            if (usernameCheck.error) throw usernameCheck.error;
            if (userCheck && userCheck.error) throw userCheck.error;
